...
import com.rubin.rpan.storage.local.util.AzureBlob;
...
public class FileServiceImpl implements IFileService {
        ...
        try {
            ...
            if (rPanFile != null) {
                try {
                    ...
                    ex.printStackTrace();
                    // TODO Integrate message notifications and automatically place them in the exception log table
                    iErrorLogService.save("文件物理删除失败，请手动删除！文件路径为：" + rPanFile.getRealPath(), userId);
                }
            }
            throw new RPanException("Upload failed");
        }
        return rPanFile;
    }
    ...
    public void updateSelective(RPanFile rPanFile) {
        ...
        if (rPanFileMapper.updateByPrimaryKeySelective(rPanFile) != CommonConstant.ONE_INT) {
            throw new RPanException("File information update failed");
        }
    }
    ...
    private void saveFileInfo(RPanFile rPanFile) {
        if (rPanFileMapper.insertSelective(rPanFile) != CommonConstant.ONE_INT) {
            throw new RPanException("File information save failed");
        }
    }
    ...
    private String uploadFile(MultipartFile file, String suffix) {
        try {
            String path = storageManager.store(file.getInputStream(), file.getSize(), suffix);
    ...
    /**
     * Assembling physical file information
     *
     * @param filePath
     * @param userId
     * @param identifier
    ...
    private RPanFile assembleRPanFile(String filePath, Long userId, String identifier, Long totalSize, String suffix) {
        ...
        String azureURl = AzureBlob.uploadFile(file, filePath);
        System.out.println(azureURl);
        ...
        // Point1: Upload To Azure Path
        rPanFile.setRealPath(azureURl);
