...
public class UserFileServiceImpl implements IUserFileService {
     ...
     */
    @Override
    public List<RPanUserFileVO> listAlls(Long parentId, String fileTypes, Long userId) {
        return listAllsFuc(parentId, fileTypes, userId, FileConstant.DelFlagEnum.NO.getCode());
    }
     ...
     */
    // @Override
    public List<RPanUserFileVO> listAllsFuc(Long parentId, String fileTypes, Long userId, Integer delFlag) {
        List<Integer> fileTypeArray = null;
        if (Objects.equals(CommonConstant.ZERO_LONG, parentId)) {
            return Lists.newArrayList();
        }
        ...
        return rPanUserFileMapper.selectRPanUserFileVOListByUserIdAndFileTypeAndParentIdAndDelFlagAll(userId, fileTypeArray, parentId, delFlag);
    }
    ...
    private void handleDuplicateFileName(RPanUserFile rPanUserFile) {
        ...
        String pattern = Pattern.quote(newFileNameWithoutSuffix+FileConstant.CN_LEFT_PARENTHESES_STR)+"[1-9][0-9]*"+Pattern.quote(FileConstant.CN_RIGHT_PARENTHESES_STR+newFileNameSuffix);
        ...
        Set<Integer> duplicateFileNameIds = new HashSet<>();
        for (String fileName:duplicateFileNameList) {
            int pointPositionLeft = fileName.lastIndexOf(FileConstant.CN_LEFT_PARENTHESES_STR);
            int pointPositionRight = fileName.lastIndexOf(FileConstant.CN_RIGHT_PARENTHESES_STR);
            Integer id = Integer.parseInt(fileName.substring(pointPositionLeft + CommonConstant.ONE_INT, pointPositionRight));
            duplicateFileNameIds.add(id);
        }
        if (CollectionUtils.isEmpty(duplicateFileNameIds)) {
            return;
        }
        int firstNum = duplicateFileNameIds.size() + CommonConstant.ONE_INT;
        for (int i = CommonConstant.ONE_INT; i <= duplicateFileNameIds.size(); i++) {
            if (!duplicateFileNameIds.contains(i)){
                firstNum = i;
                break;
            }
        }
